...
- **`Payment terms`.** How long a customer has to pay. Each row shows the name, {badge:default|outline} on the one new invoices start with, and `Due 30 days after the issue date` or `Due the day it is issued`. {button:Add term|outline|plus} at the right, and {button:Make default|ghost} on each row. A customer with a special arrangement is given their own terms on the Customers page, and a vendor theirs on the Vendors page; the default here is what every other customer gets.
