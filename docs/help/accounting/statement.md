...
- **The statement.** `Date`, `Detail`, `Company` when you keep more than one, `Charges`, `Payments` and `Balance`. The first row is `Balance forward`, what the customer owed the day before the period. Then each invoice issued in the period, `INV-0009 Invoice · due 2026-10-01`, under `Charges`, and each payment, `INV-0009 Payment · check`, with its memo, under `Payments`, in date order with the balance after each line. A credit memo is a `Payments` line too, `INV-0009 Credit memo · CM-0001 · Two bags returned`, because it reduces what is owed the way a payment does, without being money received. The number is a link to the invoice. The last row is `Closing balance`: the charges and payments of the period added up, and what is owed at the end.
