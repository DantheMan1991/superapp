...
Owners do everything on this page and are the only ones who see the `Archived` list. Staff get the page with no buttons on it and this line at the top: `Only an owner can change these. Everything set up here appears on every record.` Each row shows them the field's name and, under it, its hint, or its reference when it has no hint. They see no kind, no choice count and no badge, and no archived field at all. The outside accountant sees exactly what staff see. This page was already owners-only for everybody else, so nothing on it changed for them.
