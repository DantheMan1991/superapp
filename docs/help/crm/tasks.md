...
Owners and staff get the same page and can do everything on it. The outside accountant can open it, and can also tick a follow-up off, reopen one, hand it over and change its date. That is the one part of CRM where accountant access is not read-only, and every other button in CRM answers `Accountant access to this module is read-only.` A follow-up raised on a record with `Restrict this record` turned on is still listed here for everyone, words and all, and its `On this record` link then opens a page that shows staff nothing. The page is not there at all when CRM is switched off for your business.
