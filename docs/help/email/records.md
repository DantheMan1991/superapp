...
3. Check `{{customer}}`, or `{{vendor}}` on a bill. You cannot change it here. When nobody matched you see `Not matched to anyone on file — add them first, then draft again.` and {button:Create draft invoice|primary} cannot be clicked. Add the {{customer|lower}} or {{vendor|lower}} in Accounting, then come back and draft again.
4. Check `Dates`. It shows the date the record will carry, then the due date after an arrow, or `· no due date`. A record with no due date of its own is given one when it is created, from the customer's payment terms (or your default) for an invoice, or the vendor's for a bill, when they have any.
