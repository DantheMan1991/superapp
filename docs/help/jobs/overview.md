...
| `From the drawings` | Only on a measured line: a row per sheet — `A-101 · 59.026 sf · 1 trace · Permit set`, with *superseded issue* when that sheet has been reissued since and *measured since* when its scale moved — then the measured total against what the line says, and {button:Open the drawings|ghost}. |
...
**It remembers what you charged last time.** Type a line with no price — `tile labour 320 sf` — and underneath the box it tells you what that line cost the last time you priced it: `Last priced 3.50/sf · 24-108 · 3 weeks ago`. Press {key:Tab} to take that price and add the line; press {key:Enter} to add it with no price, as you always could. It looks at every estimate line your business has ever written, so it works from the second time you price something and gets better the more you estimate. **A remembered price is per its unit**: type `tile labour 40 sy` and a price you last charged per square foot is not offered, because 3.50 a foot is not 3.50 a yard; leave the unit off and the line takes the memory's unit along with its price.

- **It only ever fills a blank.** Type a price and nothing is suggested — you are the one pricing the job.
- **It matches the same line, not a similar one.** Capitals, commas and dashes are ignored, so `Tile, labour.` and `tile labour` are the same line to it. `tile labour, second floor` is not, and neither is a typo — you get no suggestion rather than a guess. **A wrong price offered confidently is worse than none**, because it goes out in a proposal.
...
**{button:Paste a takeoff|outline|clipboard-paste}** takes a whole block at once: one line each, in the same words, or straight out of a spreadsheet. Every line is shown in a preview before anything is added — quantity, unit and unit cost as they were read — and any line it could not read is marked in red and counted on the button, which reads *Add 4 lines, leave out 1*. Nothing is added until you press it, and the lines land in the item the select names. **A pasted takeoff is priced from memory too**: any line that came with no price and that you have priced before — per the same unit, so a line pasted in `sy` takes no price you charged per `sf` — is filled in, and the dialog says how many — *12 priced from what you charged last time. Check them — a price can be a year old.* This is the point of it: a takeoff off a spreadsheet is descriptions and quantities, and the prices are what you would otherwise type forty times.

**{button:From the model|outline}** brings the takeoff in off your model. Export a wall schedule, a material takeoff or a framing schedule from Revit — **File → Export → Reports → Schedule**, with *Export title* and *Export column headers* kept — and press {button:Choose the file|outline} to drop the `.txt` on the dialog, or paste it and press {button:Read it|primary}. It reads the schedule as **things with quantities**: every distinct entry in the column that names things (*Family and Type* on a wall schedule, *Material: Name* on a material takeoff, *Type* on a framing schedule), with its count and every figure the schedule gives it added up — *Basic Wall: Interior - 2x4 Wood Stud · 22 in the schedule · Length 412 lf · Area 3,708 sf*. The header line says how many things across how many rows, which column named them, and how many total rows or unnamed rows were left out. Beside each thing is a drop-down:

- **As an assembly** — every assembly in your library this thing could drive, each with the size it would be dropped at: *Drywall, hang and finish — at 3,708 sf*. An assembly per `sf` is dropped at the row's area, per `lf` at its length, per `ea` or per nothing once for each one counted. Pick one and the item comes in with its lines scaled to that figure, exactly as {button:Add an assembly|outline|package} would make it, and the row says what one of it costs at the size it was saved.
- **As a line** — the thing as a plain line by whichever of its figures you choose, or by its count. Priced from what you charged last time when the price book has it, and left for you when it does not.
- **— leave it out** — nothing happens for that row.

**It remembers what a name means.** Tick `remember` beside an assembly you pick, and the next takeoff off the same model chooses it for you, marked *remembered*. A name whose words match an assembly's — *Wood stud* in *Basic Wall: Exterior - 2x6 Wood Stud* — is suggested with *looks like* beside it and ticked to be remembered; check it before you bring it in. The names an assembly answers to are listed on its own page under *What the model calls it*, where you can add or forget them.

Press {button:Bring in 2 items and 1 line|primary} — the button counts what you chose. The items land at the bottom of the estimate with their lines underneath, the loose lines below them, and nothing is saved until you save the estimate as usual. A message says how many lines have no cost code on this {{project|lower}} and how many were priced from memory. **Anything it could not bring in is named with the reason** — *Wood stud is per lf and the schedule has no length for it* — so nothing goes quietly missing. With no assemblies saved yet the dialog says so, and everything comes in as lines.

**Framing and lumber.** A framing schedule with a *Count* column and a *Cut Length* column is named by type **and** length — *2x10 · 14' - 0" · 46 in the schedule* — because forty-six 2x10s at 14' and twelve at 12' are two things on a lumber list, not one 2x10 with a total. Bring each in *as a line* by its count and you have the lumber list, priced per piece from memory; map each to an assembly per `ea` and the pieces come in priced. The **Named by** selects above the rows say which column names things and which second column, if any, splits them: set the second to *— nothing* and every 2x10 is one row again; set it on any schedule to split its things by another column.

**Export it so the figures are right.** A schedule exported *itemised* — every instance its own row — adds up correctly here. One exported grouped by type adds up correctly when *Calculate totals* is on for the figure columns. Grouped without totals, a row shows one element's figure beside a count of many, and no software can tell that from a total — the count is shown beside every figure so you can.
...
- **What the model calls it** — on an assembly's own page, under the allowance box: the names a schedule off your model uses for this thing, *Basic Wall: Interior - 2x4 Wood Stud*, *Gypsum Wall Board*. Type one and press {button:Add|outline}; the cross beside a name forgets it; {button:Save|primary} writes the list with the rest of the assembly. A takeoff off the model that meets one of these names drops this assembly in without asking, and picking it in the takeoff dialog with `remember` ticked adds the name here. **A name means one assembly** — typing it on a second assembly moves it there.
...
- {button:Print proposal|outline|file-text} opens the proposal as a PDF in a new tab — **whichever document `What it is` names**, so you never have to think about it — and {button:Open brochure|outline|book-open} opens the same document on screen, where a {button:Print|primary} button in the corner prints it from your browser. Both show the estimate **as saved**; it saves itself about a second after you stop typing, so watch for `Saved` in the bar along the top first.
  - **`A price sheet`** — one running list, numbered straight through, with the parts of the bid as rows of their own: *Infrastructure*, *Structure*, *Mechanical*, *Finishes*, *General conditions*, whatever you head yours with. Each item is one row at one price, with its note beside the name — `Well  Allowance`, `Roof trusses  Supplied by others`. **Rows at `$0.00` are printed, not dropped**: on a real sheet a third of them are, and they are where the client reads what is *not* included. The headings come from each item's `Section`, and the numbers run through the headings too, so *"look at 102"* means something on the phone. {button:Open price sheet|outline|book-open} opens it on screen; {button:Print proposal|outline|file-text} gives the PDF.

    An item with no section, on a sheet where others have one, is headed **Other** rather than left to look like part of the section above it.

- **A brochure's PDF takes a few seconds the first time after a quiet spell**, because a brochure and a price sheet are printed by a browser on the server rather than drawn like the letter. If it comes back with a page saying it could not be printed, the proposal itself is fine: {button:Open brochure|outline|book-open} and use the {button:Print|primary} button, choosing **Save as PDF** as the destination. Tell us, though — that page means something needs setting on our side, not yours.
...
| `Pitch` on an area, as rise per 12 or in degrees — the select beside the box says which | `as a roof 100.4 sq ft` — the plan area times the roof's slope (6:12 is 1.118, and so is 26.6°). Blank means flat. |
...
Hover a figure and it says its working — `93.5 sq ft less 3.7 sq ft in 1 opening`, `11 ft × 9 ft`, `89.8 sq ft of plan at 6:12` (or `at 30°`, as you said it). Nothing is stored: every figure is worked out from the points, the scale and what you typed, so setting the scale again corrects all of them at once.
...
| `Estimate` | The {{project|lower}}'s draft and sent estimates, **starting on the one this sheet already feeds** — the estimate your last push from here went onto — or the only one there is. An accepted one is the agreement and cannot take a quantity. |
...
| `The new line`, `Cost code` | For a new line: what it is and its code. Its unit is the measurement's; its notes say *From the takeoff.* **It is priced from memory** when you have priced the same words before, per the same unit: type `tile` and the line comes in at what you last charged per square foot for *Tile*, its basis reading *your last price, on 24-108*, and the toast says so — *Priced from memory — $4.20/sf · 24-108 · 3 weeks ago. Check it: a price can be a year old.* A price you charged per another unit, or as a lump, is not a rate for this line and it stays blank. |

{button:Add the line|primary} or {button:Set the quantity|primary}: **the line's quantity becomes the total** — a push states, it never adds — and the toast says what went where, and what a new line was priced at when the memory priced it. The row then carries a chip per line it stands behind: `→ EST-2 · Flooring, kitchen · 59.026 sf`, reading the line as it is now on the estimate, and `· around it` or `· as a wall` when it stands there by a figure other than its own — one room's row can carry the flooring, the baseboard and the slab at once. Set the scale again and, if this measurement's own figure moves, the chip says `measured since`; push again to bring the line up to date, or leave it. Two measurements pushed onto one line each watch their own share of it, so the chip stays quiet until something on the drawing really moves. {button:×|ghost} beside the chip lets the measurement go without touching the line. Taking the line off the estimate leaves the measurement, marked `Line gone`.
