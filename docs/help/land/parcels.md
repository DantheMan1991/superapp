...
## How to find out which {{zone|lower}} you are standing in

1. On your phone, standing in a field, click {button:Which {{zone|lower}} am I in?|outline}. It reads `Finding you…`.
...
6. Click {button:Add 20 {{zone|plural|lower}}|primary}. It reads `Adding…`, then you see `Added 20 {{zone|plural|lower}}`, the dialog closes and the list refreshes. Nothing is saved until you click it. If any ticked row is refused, nothing is saved at all, and the message names the row.
...
| --- | --- |
| `Add a parcel first, so a {{zone|lower}} has somewhere to be.` | You clicked {button:Paste a list|outline|sparkles} with no {{parcel|lower}} yet. |
...
| `You are not inside any mapped {{zone|lower}}. Trace its boundary and this will find it.` | Your position is not inside any {{zone|lower}} with a traced boundary. |
...
Owners add, combine and find {{parcel|plural|lower}}. Everyone gets {button:Which {{zone|lower}} am I in?|outline} and {button:What is here?|outline} — they write nothing, and the person standing in the field is often not the owner.
