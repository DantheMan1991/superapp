...
- **A feature's panel.** Click a fence, gate or other feature on the map and its panel opens under the key, and the page scrolls to it — on a phone the panel sits well below the map, and without that you would tap a fence and see nothing happen. Clicking a row in the list below does not scroll, because you are already looking at the row you clicked. Switching to another feature starts its panel fresh, so anything you had open on the last one closes: its name or kind, {badge:Built|primary}, {badge:Planned|outline} or {badge:Removed|secondary}, its length, its details and its notes, with {button:Take me there|outline}, {button:It is built|outline|check} or {button:Mark as removed|outline} or {button:It is back|outline}, {button:Edit details|outline} and {button:Delete|destructive|trash}. Under the map, {button:Move the points — [name]|outline} redraws it, or {button:Draw — [name]|outline} for one that has no shape yet. Click a {{zone|lower}} and it is selected for drawing. Click empty ground to clear the selection.
- **The list of features.** Under the map, everything drawn, under headings you open. Four controls sit above it. `Group by kind` to begin with, or `Group by plan`, or `No grouping` for one flat list. `Every kind` or one kind. A state filter that starts on `Built and proposed` and can show `Built`, `Proposed`, `Removed` or `Every state`. And `Find by name`, which matches the name or the kind as you type. A count reads `15 shown` or `7 of 15`.
- **The headings.** Each reads what it holds, how many, and how much: `Fence  7 · 7,714 ft`. A heading with nothing in it that has a length, such as four gates, shows the count alone rather than `0 ft`. Headings start closed, so a {{parcel|lower}} with forty things drawn on it is six lines rather than forty. Click one to open it. Grouped by plan, each layout you ran is its own heading under the name you gave it, and everything drawn on its own sits under `Not in a plan` at the bottom. When there is only one heading it is already open, and typing in `Find by name` opens every heading that matched.
- **The rows.** On a computer, columns you can sort by clicking: `What`, `Kind`, `State`, `Length`. On a phone, a card each carrying the same four things. Names ending in a number are ordered by that number, so `Paddock 2` comes before `Paddock 10`. Clicking a row or a card selects it on the map. Check boxes let you delete several at once, after `Delete 3? This cannot be undone`; the check box on a heading takes everything under it. **Only what you can see is counted** — closing a heading takes its ticks out of the number, and opening it again puts them back. The footer totals the length, `3,400 ft in all`, noting anything not drawn yet. Points have no length and are left out.
...
- **`What it will take`.** Once the {{parcel|lower}} has a plan: `Counted off the drawing. Nothing here is guessed.` `Material`, the quantity `From the drawing`, and `Price`, with {badge:List saved|secondary} or {badge:Not taken off yet|outline} on each plan. A table on a computer, a card per material on a phone. See how to use it, below.
...
## How to find one thing among many

1. Leave `Group by kind` as it is and click the heading you want. `Fence  7 · 7,714 ft` opens to seven cards.
2. To see one layout's work on its own, switch to `Group by plan`. Dividing ground creates a plan named after the prefix you typed, so everything that one decision drew — the dividing fences, the lane fences and a gate for each {{zone|lower}} — sits under one heading. Anything you drew by hand is under `Not in a plan`.
3. To go straight to something by name, type into `Find by name`. It matches the name and the kind, so `north` finds `North lane fence 1` and `gate` finds every gate. Every heading holding a match opens while you are typing.
4. `No grouping` puts everything back into one list.

Nothing here changes the drawing. Grouping, filtering and finding only change what you are looking at.
