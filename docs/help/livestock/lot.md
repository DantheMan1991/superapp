...
- **`Fed`.** What feed has cost, with a badge saying how it is known: {badge:Measured|outline} for feed issued to this one by name, {badge:Allocated|primary} for a share of a shared feeder. Rest on the badge to see what that means. Once some of the group has been processed, a line underneath reads `$43.15 left with what was processed · $98.52 still on this lot.` The second figure can read below zero, as `−$300.00`: what left carried the animals' own purchase price as well as their feed, and the figure above it is feed alone. When part of the cost is a share of a shared feeder, another line reads `$85.00 issued by name, $56.67 a share of a shared feeder`, and once something has been processed it adds that a share stays with the pen rather than travelling with the meat.
