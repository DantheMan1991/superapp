...
- **The selected section.** A card headed with the section's kind, {button:Remove section|ghost|trash}, its own fields (below), and under them `Layout and look`, the same on every kind: `Width` ({button:As designed|outline}, {button:Text column|outline}, {button:Page|outline} or {button:Full width|outline}; a Photo section and a Slideshow keep their own width setting instead), `Spacing` ({button:As designed|outline}, {button:Tight|outline}, {button:Normal|outline} or {button:Airy|outline}; the Big headline has `Height`, {button:Compact|outline}, {button:Standard|outline} or {button:Tall|outline}, instead), `Alignment` ({button:As designed|outline}, {button:Left|outline} or {button:Centred|outline}) and `Background` ({button:As designed|outline}, {button:None|outline}, {button:Tint|outline}, {button:Brand colour|outline}, {button:Dark|outline} or {button:Photo|outline}). `As designed` is how that kind of section looks on its own; every choice is a preset that fits a phone as well as a laptop. A dark background or a photo turns the words white; the brand colour uses your brand's own contrast. Choosing `Photo` adds a `Background photo` picker; the photo is darkened so the words stay readable and, being decoration, needs no description. A Big headline or an About section with a photo also gets `Photo side` ({button:Right|outline} or {button:Left|outline}).
  ...
  - *Book a time*: offered only while Scheduling is switched on; otherwise the button is grayed out and reads `Switch on Scheduling under Modules to take bookings.` when you rest on it, and a section added earlier shows an amber note saying it offers no times. `Heading`, `Note`, `What is being booked` (as it reads on your calendar and in the email: `Farm visit`, `Consultation`, `Fitting`), `How long` ({button:15 min|outline} to {button:2 hours|outline}), `Days` (seven buttons, `Sun` to `Sat`; times are offered on the ones pressed), `From` and `To` (the hours on each of those days), `Notice` ({button:None|outline}, {button:2 hours|outline}, {button:A day|outline}, {button:Two days|outline}: how soon a visitor may book), `How far ahead` ({button:A week|outline} to {button:Two months|outline}), `Button` (blank reads `Book`), `Ask for a phone number`, and `After booking`, the line shown once a time is booked. The times offered on the site are the open ones on your `Bookings` calendar in Scheduling inside these days and hours; put anything on that calendar to keep a time from being offered. Each booking goes on that calendar with the person on it, becomes a contact and a follow-up, and is emailed to you. See [Your website](website.md).
...
- **`Preview of the saved draft`.** The right-hand side shows the page as the renderer draws it, followed by `Click a section to edit it.` It reads `(save to see your changes)` while you have unsaved edits. Three buttons at the right, {icon:monitor} `Desktop`, {icon:tablet} `Tablet` and {icon:smartphone} `Phone`, show the page at the whole pane's width, at a tablet's (820 pixels) or at a phone's (390 pixels), in a dark frame; the one you choose is remembered in this browser. {button:Reload|ghost|refresh} redraws the preview. Move the pointer over the preview and each section shows a dashed outline in your primary color; click one and it is selected in `Sections`, its fields open below the list, and the outline turns solid. Select a section in the list and the preview scrolls to it and outlines it. A button or a link inside a section selects the section rather than going anywhere; the menu at the top still moves between pages. The preview shows the page as it was last saved, so after you add, move or remove sections, save before you rely on a click to pick the right one.

## How to change what a page says

1. Click a section in `Sections`, or click it in the preview. Its fields open below the list.
2. Change the words. The list's summary line updates as you type. Press {icon:smartphone} `Phone` above the preview to see how the page reads on a phone once it is saved.
