...
Owners can build, publish, unpublish, rewrite, change the details, change the address and remove messages. Staff can open the page, see the addresses, the details, the messages and the visitors, and open the draft preview and the live site; they cannot change anything. Accountants see the same read-only view. Anyone on the internet can send a message through the form on a published site; nobody needs to sign in for that.
