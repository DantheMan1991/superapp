...
- **`Header and footer`.** Under the heading: `What every page shares: a line across the top, a button in the header, your pages elsewhere, and the footer's columns. Saved changes show on the site straight away.` Four blocks, then {button:Save|primary} and {button:Discard changes|ghost}. Save stays grayed out until something changed, and while any link in the card has a red line under it.
