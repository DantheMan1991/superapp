...
- **`Header and footer`.** Under the heading: `What every page shares: a line across the top, a button in the header, your pages elsewhere, and the footer's columns. Saved changes show on the site straight away.` Five blocks, then {button:Save|primary} and {button:Discard changes|ghost}. Save stays grayed out until something changed, and while any link in the card has a red line under it.
  ...
  - *Social links*: `Your pages elsewhere, shown as icons in the footer. Paste the address of your page and the network fills in from it.` {button:Add a link|outline|plus} adds a row, up to eight: a network list (`Facebook`, `Instagram`, `YouTube`, `TikTok`, `LinkedIn`, `X`, `Pinterest`, `Another site`), an `Address` box, and, for `Another site`, a box reading `What to call it, such as Etsy shop`. A new row starts as `Another site`; paste an address on one of the seven networks and the list changes to it by itself. {icon:trash} removes a row. On the site each known network is a round icon in the footer, and `Another site` is your own words in a rounded pill; every one opens in a new tab.
