...
- **`Build your website`.** A short explanation, then the form: the address, `Phone`, `Email`, `Address`, `Hours` and `About the business` (a few lines in your own words about what you raise or sell, how, and who buys it; the writer's best source, never shown on the site as written). What gets built is your industry's template: a homestead farm gets five pages (Home, Shop, Visit, About, Contact) with a warm look, a header button, footer columns and starter pictures drawn in your brand's colours, and every other business gets the general three (Home, About, Contact). The words on every page are written for your business from your brand kit and the details below; a Price list, a What's on or a Book a time section is included only when Retail or Scheduling is switched on.
...
- **The first card.** The site's name with a badge, {badge:Draft|secondary} or {badge:Published|success}, and a line that reads `Not on the internet yet.` or `On the internet since [date].`, followed by `Built from the [template] template.` and then `The words were written by Yosher's assistant from your brand kit; read them before you publish.` or `The words are the template's own; read them before you publish.` The template's own words are its starter text, used when the assistant could not write for you; they read on their own, and say `[your business]` where the assistant would have said more.
...
- **`Pages`.** Under the heading: `Drag to set the menu order. A page's words wait for Publish; its place in the menu shows at once.` One row per page: its title, its address such as `/about`, how many sections it has, `2 photos without a description` in amber when a photo on the page's draft has no description (see [Editing a page](page-editor.md)), `3 photos to take` when places for a photo on the page are empty or hold one of Yosher's drawn stand-ins (see [Photos to take](shot-list.md)), and `published` or `draft only`. {button:Preview|ghost} opens that page's draft in a new tab. Owners also see the {icon:grip-vertical} drag handle, {button:Edit|outline|pencil}, which opens the page in the editor (see [Editing a page](page-editor.md)), and {icon:trash} on every page but the home page. Under the list, {button:Add a page|outline|plus} opens a `Title` and an `Address` with {button:Add page|primary} and {button:Cancel|ghost}; the address fills in from the title and the line under it reads `Will be at /services` or says why it cannot be used.
- **`Photos`.** Under the heading: `Where a photo belongs on your pages, what to take there, and a way to put it in from your phone.` A card with one line on where things stand, such as `3 of the 11 places for a photo have one: 6 still to take, 2 drawn stand-ins to replace.` or `Every one of the 11 places for a photo has one.`, and {button:Open the shot list|outline|camera}, which opens [Photos to take](shot-list.md): every place on every page, what to photograph there, and a way to take or choose the photo and put it straight in.
- **`Messages`.** Under the heading: `What people sent through the form on your site. Each one is a contact and a follow-up in your workspace, and was emailed to you.` One row per message, newest first, up to the last thirty: the sender's name, the date it arrived, a {badge:booking|outline} badge with `[what is being booked], [day], [start] to [end]` under the name when the message is a booking (see *How bookings from your site reach you* below), a badge reading {badge:to reply|outline} while its follow-up is open, {badge:replied|outline} once the follow-up is marked done, or {badge:follow-up removed|outline} if the follow-up was deleted; their email and phone as links you can click to write or call; `from /contact` when the form was on a page other than the home page; the answers to any questions of your own, one line each as `Question: answer`, and the first line of the message with `Show the whole message` under it (`Show less` folds it again). {button:Follow-up|outline} opens the follow-up in Work, when Work is switched on. {button:Contact|outline} opens their record in CRM, when CRM is switched on. The follow-up and the contact exist either way; the buttons only appear where there is a page to open. Owners also see {button:Remove|ghost|trash}; your browser asks `Remove the message from [name]? The contact and the follow-up it made stay where they are.` A line under each row says where the message went: `Contact: [name].` or `Saved as [name] in your contacts.`, `The contact it made has since been removed.`, and `Emailed to the site's email address.`, `Emailed to the owners; add an email to the site's details to send it there instead.` or `Not emailed: the site has no email address and no owner has one.` With no messages yet the card reads `No messages yet. When someone fills in the form on your site, it lands here.`
- **`Visitors`.** Under the heading: `How many people looked at your site in the last thirty days, and which pages.` The card opens with a line such as `143 visitors and 412 page views in the last 30 days.`, then a bar for each of the thirty days (hold the pointer over one to read the date, its visitors and its views; the first and last dates are printed under the bars), then a table with a row per page: `Page` (its title and its address), `Views` and `Visitors`, most viewed first. Under the table: `A visitor is a browser on its first page of the day; a view is every page it opens. Nothing about the person is kept.` Until anything is counted the card reads `No visits counted yet. Every page a visitor opens on the published site counts here.` Only the published site counts; your own draft preview does not.
- **`Details on the site`.** `Name in the header`, `Phone`, `Email`, `Address`, `Hours` and `About the business`, the same fields as when you built the site (the about lines feed the writer the next time you click {button:Rewrite the words|ghost}), with {button:Save|primary} and {button:Discard changes|ghost}. The header name is blank by default, which uses your brand kit's business name. Under `Address`, a second line says where the map stands: `On the map as [the address as the map service read it].`, or in amber `Not on the map: the address could not be placed. Check it and save again.` or `Add an address to put the business on the map.` (see *How the map finds your address* below). These details show on the live site the moment you save; there is no need to publish again.
- **`Header and footer`.** Under the heading: `What every page shares: a line across the top, a button in the header, your pages elsewhere, and the footer's columns. Saved changes show on the site straight away.` Five blocks, then {button:Save|primary} and {button:Discard changes|ghost}. Save stays grayed out until something changed, and while any link in the card has a red line under it.
  - *Announcement bar*: `A line across the top of every page, in your brand color. Holiday hours, a market date, a sale.` A `Show the bar` switch, `Text` (up to 120 characters) and `Link` (optional; `Optional. Where the line leads when it is clicked.`). The bar shows on every page while the switch is on and there is text; switching it off keeps the text for next time. On the site the bar is a band in your brand color across the very top, and the text is underlined when it has a link.
  - *Logo size*: `How big your logo is drawn at the left of the header, on every page.` {button:Small|outline}, {button:Medium|outline} or {button:Large|outline}; medium is the standard. Large suits a wordmark or a round mark that reads small; on a phone the logo never grows past a third of the header. A brand kit with no logo shows the business name instead, at one size.
  - *Header button*: `A button at the end of the menu on every page. Leave the label blank for no button.` A `Label` (up to 40 characters, such as `Book now`) and `Goes to`, with the line `A page on this site such as /contact, a full https:// address, or a mailto: or tel: link.` under it. On the site it is a rounded button in your brand color after the menu. On a phone the pages fold behind a {icon:menu} `Menu` button at the right of the header and your button stays beside it; tapping {icon:menu} opens the list of pages under it, {icon:x} closes it, and choosing a page closes it too. With only the home page in the menu there is nothing to fold and the button stands alone.
  - *Social links*: `Your pages elsewhere, shown as icons in the footer. Paste the address of your page and the network fills in from it.` {button:Add a link|outline|plus} adds a row, up to eight: a network list (`Facebook`, `Instagram`, `YouTube`, `TikTok`, `LinkedIn`, `X`, `Pinterest`, `Another site`), an `Address` box, and, for `Another site`, a box reading `What to call it, such as Etsy shop`. A new row starts as `Another site`; paste an address on one of the seven networks and the list changes to it by itself. {icon:trash} removes a row. On the site each known network is a round icon in the footer, and `Another site` is your own words in a rounded pill; every one opens in a new tab. A mark here is a link you chose to show visitors; the accounts you POST from are kept separately on [Your social accounts](social-accounts.md), and that page can put one here in a click.
  - *Footer*: `Your name, details and social links are always in the footer. Add up to three columns of your own beside them: links to pages, hours, a few words.` On the site the footer is a band in your brand color, its words in the color that reads on it, with the column headings in small capitals; the header stays at the top of the window as a visitor scrolls. {button:Add a column|outline|plus} adds a panel headed `Column 1`, `Column 2` or `Column 3`, each with a `Heading` (up to 40 characters), `Text` (up to 300, line breaks kept), and `Links`: {button:Add a link|outline|plus} adds a row of `Label` and `Link`, up to six, with {icon:trash} to remove one; {button:Remove column|ghost} takes the whole panel off. `Footer line` (up to 160 characters) sits under everything: `Under everything, beside the © line, which is always there.` On the site, with no columns the footer is one row: your business name and tagline on the left, the phone, email and social links on the right. With columns it is a grid: your name, tagline, address, phone, email and social links in the first column, then yours. Under either, `© [year] [your business]` and the footer line.
  - Under any link box, a red `A page on this site such as /contact, a full https:// address, or a mailto: or tel: link.` means the link is not one of those four; under an `Address` box, `A full address that starts with https://.` means the same for a social link. Staff see the card as a summary: `Announcement bar`, `Header button`, `Social links` and `Footer`, with `Only an owner can change these.`
...
1. In `Header and footer`, turn on `Show the bar` and type the line in `Text` for an announcement across the top of every page; add a `Link` if it should lead somewhere. Turn the switch off to hide it and keep the words.
2. Type a `Label` and where it `Goes to` for a button at the end of the menu, such as `Book now` to `/contact`. Blank the label to remove the button.
3. Under `Social links`, click {button:Add a link|outline|plus} and paste the address of your page; the network fills in from it. For a site not on the list, leave `Another site` and say what to call it.
4. Under `Footer`, click {button:Add a column|outline|plus} for each column you want, give it a `Heading`, `Text` or `Links`, and type a `Footer line` if you want one.
5. Click {button:Save|primary}. You see `Header and footer saved. They show on the site straight away.` Open the live site to check it; there is no need to publish again.
...
Your contact page carries an enquiry form: `Name`, `Email`, `Phone (optional)`, any questions you added to it in the editor, and `Message`, with a button that reads `Send` unless you changed it. A question you marked `Must be answered` stops the message until it is answered; the others show `(optional)`. A site built before the form existed does not have one until you add it: open the contact page in the editor and add an {button:Enquiry form|outline} section (see [Editing a page](page-editor.md)), then publish.
...
## How bookings from your site reach you

A `Book a time` section on a page (see [Editing a page](page-editor.md)) shows visitors the open times inside the days and hours you set, taken from your `Bookings` calendar in Scheduling: Yosher makes that calendar the first time you save a page with the section, owned by the business and shared with everyone in it as `Can edit`. Anything on that calendar blocks the time it covers, so a member keeps a time from being offered by putting an event on it. Bookings need Scheduling switched on; without it the section offers no times.

When a visitor picks a day and a time, gives their name and email, and presses `Book`:

1. The time is checked again at that moment. If somebody took it in between, the visitor reads `That time was just taken. Pick another.` and the times left.
2. They become a contact, matched by email, as a message does; with CRM on, the record's source reads `website`.
3. The booking goes on the `Bookings` calendar as `[what is being booked]: [name]`, with the visitor on it and everything they sent in its description.
4. A follow-up called `Confirm the booking with [name]` is raised in Work, due today.
5. It appears under `Messages` on this page with a {badge:booking|outline} badge and the time, as `[what is being booked], [day], [start] to [end]`.
6. You are emailed `[name] booked [what is being booked] from your website`, from the site's email address if there is one, otherwise to every owner. Reply to that email to confirm with them; Yosher does not email the visitor itself.
...
## How the map finds your address

A `Find us` section on a page (see [Editing a page](page-editor.md)) shows a map of the `Address` in `Details on the site`. When you save the details, Yosher asks the United States Census Bureau's address service where that address is and keeps the answer; the line under `Address` then reads `On the map as [the address as the service read it].` If the service could not place it, the line reads `Not on the map: the address could not be placed. Check it and save again.`: make the address more specific (the street type, the city, the ZIP) and save again. With no address it reads `Add an address to put the business on the map.` The map itself is a picture Yosher draws from public-domain USGS maps with your primary color as the pin, so nothing on your page is fetched from anywhere else by your visitors. The maps cover the United States only.
...
| `Header and footer saved. They show on the site straight away.` | The announcement bar, header button, social links and footer were saved and the live site already shows them. Blank rows were dropped. |
| `The header button needs a link.` / `The header button needs a label.` | The button has one half; give it the other, or blank both for no button. |
| `The announcement bar's link needs to be a page on this site such as /contact, a full https:// address, or a mailto: or tel: link.` | The bar's `Link` is not one of the four kinds. The same message names `The header button's link` or `The footer link "[label]"` when it is one of those. |
| `Every social link needs a full address that starts with https://.` | An `Address` under `Social links` is not a web address; paste it from your browser's address bar. |
| `The footer link "[label]" needs a link.` / `The footer link to [address] needs a label.` | A footer link has one half; give it the other, or clear both and the row is dropped. |
| `A page on this site such as /contact, a full https:// address, or a mailto: or tel: link.` (in red, under a link box) | Typed under the box as you go: the link is not one of the four kinds, and Save waits until it is. |
| `A full address that starts with https://.` (in red, under an `Address` box) | The same, for a social link. |
...
| `Booked: [day], [start] to [end]` | What a visitor sees on your site after booking a time; your `After booking` line follows it. |
| `That time was just taken. Pick another.` | On your site: somebody booked or blocked that time between the visitor choosing it and pressing `Book`; the times left are shown. |
| `Finding open times…` / `No open times at the moment. Use the phone or email on this page.` | On your site: the open times are being fetched, or there are none inside the section's days, hours and horizon. |
| `Booking isn't open right now. Use the phone or email on this page.` | On your site: the site is unpublished or Scheduling is switched off. |
| `That's a few bookings from here already. Give it an hour, or use the phone or email on this page.` | On your site: the same visitor made more than five bookings in an hour. |
| `Pick a time.` | On your site, under the times: `Book` was pressed with no time chosen. |
...
| `This one is needed to send.` | On your site, under one of your questions: it is marked `Must be answered` and was left blank. |
| `Pick one to send.` | On your site, under a `Pick one` question that must be answered: nothing was chosen. |
| `Pick one of the choices.` | On your site: the answer is not one of the choices you offer. |
| `Tick this one to send.` | On your site, under a `Yes or no` question that must be answered: the box is not ticked. |
| `Keep this under 200 characters.` / `Keep this under 1,000 characters.` | On your site: a short or a long answer is over its length. |
| `No visits counted yet. Every page a visitor opens on the published site counts here.` | Nothing has been counted, or the site is not published. |
...
Owners can build, publish, unpublish, rewrite, change the details, change the header and footer, change the address, remove messages and place photos from the shot list. Staff can open the page, see the addresses, the details, the header and footer, the messages and the visitors, and open the draft preview and the live site; they cannot change anything. Accountants see the same read-only view. Anyone on the internet can send a message through the form on a published site; nobody needs to sign in for that.
