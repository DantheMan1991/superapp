...
### 2026-08-10 — The invoice becomes a document (branch `claude/invoice-pdf`)
...
- **Nothing here is public.** A PDF only ever reaches somebody already signed in to the tenant. Sending it to the customer is the next slice, and it is separate because mail goes out through the tenant's own mailbox ([ADR 0003](../decisions/0003-self-hosted-mail-over-provider-apis.md)) — invoice delivery therefore depends on the mail module being configured, which is a real coupling worth its own review
- Deliberately **no public payor view**. QuickBooks has one because it collects card payments there; payment processing for tenants' customers is out of scope by design (the PCI firewall), so for us the email plus its attachment IS the delivery story
...
- **Invoice delivery, part 2**: the PDF shipped 2026-08-10; still to do is **sending** it through the mail module plus a `sent_at` timestamp. That couples to a configured mailbox, because mail must leave from the client's own domain ([ADR 0003](../decisions/0003-self-hosted-mail-over-provider-apis.md)) — which is why it is a separate slice. A public payor view is deliberately **not** planned: QuickBooks needs one to take card payments, and payment processing for tenants' customers is out of scope by design
