...
| `project_budgets` / `budget_lines` **(SHIPPED as `job_budget_lines`; all five columns per code since 2026-09-14)** | Original, revised, committed, actual, projected — by cost code. | The five columns every construction report is made of. Actual per code arrived last, once `getBalances` could slice the ledger to one job (`withinMemberId`); projected is the greater of committed and actual, never their sum. |
...
| 4 | ~~`change-orders` — PCO → CO → approved, against a contract~~ **SHIPPED 2026-09-14** ([jobs.md](jobs.md)) | The original contract value must stop being editable before anything trusts it — and with a budget now in place, an approved change order can revise both halves at once, which is what `original + approved changes = revised` means. **The payable side arrived as slice 4b ([ADR 0065](../decisions/0065-a-subcontract-change-order-adds-lines-to-the-order-and-an-issued-orders-lines-are-locked.md)): a change order on a subcontract or purchase order adds lines to it, counted once approved and billed by the subcontractor's next application as the same rows, passing the client's change order down when there is one; an issued order's lines are locked the way a signed value is.** |
| 5 | ~~`progress-billing` — SOV, pay applications, retainage, draws~~ **SHIPPED 2026-09-14** ([jobs.md](jobs.md), [ADR 0058](../decisions/0058-a-pay-application-is-an-ordinary-invoice.md)) | **Three methods, not six.** The pilot bills fixed price monthly, AIA pay application and a home draw schedule — all percent-or-milestone against a fixed value, and one model covers all three. An issued application is an ordinary Accounting invoice; retainage is a negative line to the receivable the profile seeds, and lowering the rate releases it. Cost-plus-fee and GMAX **arrived as slice 5b the same day ([ADR 0060](../decisions/0060-a-cost-plus-application-bills-the-ledger-not-the-bills.md)): the books' tagged cost by code is the schedule, billed to date, plus the fee, capped**; **time and materials arrived as slice 5d ([ADR 0062](../decisions/0062-a-time-and-materials-application-bills-approved-hours-at-a-rate.md)): Time's approved hours tagged with the job at each person's charged-out rate or one rate on the contract, plus the books' other cost marked up with the wages accounts left out — cost plus with a rate card in place of labour cost, Time's `bill_rate_cents` finding its first reader**; **unit price arrived as slice 5f ([ADR 0064](../decisions/0064-a-unit-price-application-bills-quantities-at-the-schedules-prices.md)): the schedule with a unit, an estimated quantity and a price per item, applications by quantities installed, the invoice item by item — every method bills**; **the printout arrived as slice 5e ([ADR 0063](../decisions/0063-a-pay-applications-printout-is-rendered-from-the-frozen-certificate.md)): the certificate page and the continuation sheet as a PDF from any application's row, rendered from the frozen figures, in the shape everybody knows and in the product's own words**; **retainage held FROM subcontractors arrived as slice 5c ([ADR 0061](../decisions/0061-a-subcontractors-application-is-an-ordinary-bill.md)): a subcontractor's application against the subcontract's own lines, approved as an ordinary bill with the held share to `2120`**; the AIA-style printout shipped as 5e. |
...
| 8 | ~~`selections` — option catalogue, allowances, selection deadlines~~ **SHIPPED 2026-09-14** ([jobs.md](jobs.md), [ADR 0067](../decisions/0067-a-selection-is-a-decision-with-an-allowance-and-priced-choices-and-its-difference-moves-by-change-order.md)) | Covers the production option book and the custom selection process with one mechanism: a selection with an allowance and a needed-by date, its priced choices with one chosen, the difference raised as an ordinary change order once approved, the reminder as Work on the selection. **Per job for now; the tenant-level option book that seeds a job is 8b.** |
| 9 | ~~`drawings` — sheet sets, versions, markups, measurements~~ **9a SHIPPED 2026-09-15** ([jobs.md](jobs.md), [ADR 0072](../decisions/0072-a-drawing-set-is-an-issue-of-pages-in-documents-and-the-current-set-is-derived.md)) | The Documents industry layer, on the cabinet as it is: a set is an issue of drawings whose PDF is a cabinet document hung on the set, each page with a number a sheet, the current set DERIVED as the newest issue of every number. The browser reads the title blocks and proposes the numbers; the office confirms. **Markups shipped as 9b the same day ([ADR 0073](../decisions/0073-a-markup-is-a-vector-on-a-sheets-issue-and-a-pin-is-a-punch-item-where-it-sits.md)): clouds, arrows, notes and pins as vectors over one issue of a sheet, a pin a punch item where it sits. Measurements and the takeoff shipped as 9c on 2026-09-16 ([ADR 0074](../decisions/0074-a-measurement-is-a-markup-with-a-quantity-the-scale-is-the-sheets-and-a-takeoff-is-a-quantity-pushed-onto-an-estimate-line.md)): the scale on the sheet, a length, an area and a count as markups with points, a quantity pushed onto an estimate line. The row is done.** |
| 10 | ~~`estimating` — takeoff, assemblies, unit costs, bid~~ **SHIPPED 2026-09-15** ([jobs.md](jobs.md), [ADR 0069](../decisions/0069-an-estimate-prices-the-job-before-anybody-signs-and-accepting-it-names-the-contract.md)) | The bid, as lines of cost and price by cost code — a markup on cost or a typed unit price, overhead and profit below — with the margin in view; accepted, it sets the contract's value, and two more buttons make it the budget (cost by code) and the schedule of values (price by line, overhead and profit spread so it totals the contract sum). Units of measure and the cost code set it estimates into were settled by 5f and 3. **The proposal shipped as 10b, 2026-09-15 ([ADR 0070](../decisions/0070-a-proposal-is-the-estimate-at-its-price-and-its-words-are-fixed-with-the-money.md)): the estimate at its price, three ways, cost never printed, the words fixed with the money. Assemblies, a unit cost book and the takeoff remain.** |
| 11 | ~~`compliance` — COI expiry, lien waivers, W-9~~ **SHIPPED 2026-09-14/15** | Work raised where it lives; no new task engine. **Subcontractor documents shipped as slice 11b, 2026-09-15 ([ADR 0068](../decisions/0068-a-subcontractors-documents-hang-off-the-party-and-the-only-behaviour-a-kind-carries-is-its-expiry.md)): a document per party with an open kind and an expiry, the required list a tenant value, standing derived against today on a Subcontractors page and in a line on every order.** **Lien waivers shipped as slice 11a, 2026-09-14 ([jobs.md](jobs.md), [ADR 0066](../decisions/0066-a-lien-waiver-is-a-record-with-a-kind-and-a-through-date-and-the-gap-is-derived.md)): a record on the order — who, which kind, through when, received or not, the signed copy attached — with "paid and no unconditional waiver on file" derived from the applications' bills and the chase raised as a Work item on the order.** |
| 12 | `submittals` — submittals, RFIs, ASIs, transmittals | Commercial-weighted, so it waits for a commercial job run in anger. |
| 13 | ~~`warranty`~~ **SHIPPED 2026-09-16** ([jobs.md](jobs.md), [ADR 0076](../decisions/0076-a-warranty-claim-is-the-record-of-a-call-its-work-is-a-work-item-the-period-is-the-jobs-and-a-claim-outside-it-is-said-never-refused.md)) · ~~`bonding`~~ **SHIPPED 2026-09-16** ([ADR 0078](../decisions/0078-a-bond-is-recorded-on-the-job-and-a-job-counts-once-against-the-suretys-line.md)) · `certified-payroll` | The period on the job — months from substantial completion, the expiry derived — and a claim as the record of the call: what, where, who, when, the trade responsible, the decision with its reason; the work an ordinary Work item linked to the claim, so the digest chases it; where it stands derived; the cost read from the job cost report under the claim's code; a claim outside the period said, never refused. A Warranty tab per job and a page across jobs. **Bonding shipped the same day: the bond recorded on the job (an open kind, the surety a party, the premium recorded and never posted) and the CAPACITY across jobs — the single-job and aggregate limits from the surety's letter against the backlog on bonded work, with a job counted ONCE however many bonds it carries. `certified-payroll` remains, and may not belong in software at all.** |
| 14 | ~~`schedule` — phases, milestones, dependencies, the trade on each~~ **SHIPPED 2026-09-15** ([jobs.md](jobs.md), [ADR 0071](../decisions/0071-a-jobs-schedule-is-its-phases-as-items-on-the-business-calendar-and-a-move-pushes-what-follows.md)) | Never on the plan and every builder lives by it. A phase is an item on the business's Job schedule calendar, so the company calendar and the phone feed carry it; the pack keeps the order, the finish-to-start dependency with a lag, the trade and the status, and a move pushes what follows. **Working days, a baseline, telling the trade and a schedule template are next.** |
