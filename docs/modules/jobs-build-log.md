# Jobs — build log archive

> The earlier build-log entries for the Jobs pack — **2026-09-14 to
> 2026-09-18**, the week the pack was built — swept out of [jobs.md](jobs.md) on
> 2026-09-20 so the dossier stays readable. AGENTS.md has every session that
> touches the construction family read that dossier FIRST, and at 6,543 lines,
> 86% of it build log, its length was a tax on every future change.
> `src/lib/build-docs.ts` walks the whole tree, so this renders at `/admin/docs`
> beside the dossier with no code change.
>
> **Nothing here was edited.** The entries are exactly as they were written,
> newest first, from *Which parts of a job this business does* back to slice 0,
> the project spine — slices 0 through 13a, the estimate program's E1–E8, and the
> jobs redesign. [jobs.md](jobs.md) keeps the estimate interview run (X1 on) plus
> the data model, the seams, the decisions & gotchas and the open items, which
> are never swept because they describe the pack as it is rather than how it got
> here. Read the dossier's Decisions & gotchas first: everything in an entry
> below that still binds a future change was lifted into it at the time.
>
> **Two ways an entry below can mislead you.** A link to
> `#the-estimate-program-open-started-2026-09-16` no longer jumps — that section
> stayed in [jobs.md](jobs.md), and so did the *IF YOU ARE CHANGING THE ESTIMATE
> SCREEN* table beneath it. And the 2a redesign (E8) moved three of that table's
> rows, so an earlier entry describing the grid as a `<table>` whose keyboard
> lookup is `tr[data-group]` is the record of what was true then; it is
> `[data-row][data-group]` now, and the dossier's table is what to trust.
