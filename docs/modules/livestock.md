...
Newest first. One entry per session/PR that touched this pack. Every PR that
changes this pack MUST add an entry here (rule in AGENTS.md).

> **Older entries live in [`livestock-build-log.md`](./livestock-build-log).**
...
> at the start of every livestock session, so its length is a real cost — it was
> 3,695 lines before the 2026-09-14 sweep.
...
shorter. It had reached **3,695 lines**, and AGENTS.md tells every livestock
session to read this dossier first.

- Entries dated before 2026-09-09 moved to `livestock-build-log.md`. **Nothing
  was edited and nothing was dropped**: 64 `###` headings before, 36 here and 28
  there, every moved line byte-identical to what it was. build-docs walks the
...
- **Four things deliberately did NOT move**, because they are current design law
  and not history: "The model, settled 2026-08-27", which the header tells you to
  read before anything else; "The first version was wrong, and driving it is what
  showed that", which is why a breeding cow keeps her head and only the money
  leaves; "And it found a live safety bug", which is why a withdrawal is
  inherited at READ time through `treatmentsByLot` rather than copied at split
  time; and the data model, seams, decisions and open items
- The archive carries the same vocabulary warning the dossier does: entries there
  written before 2026-08-27 say *"an individual is a lot of one"*, and that
  sentence is retired
...
| `livestock_lot_members` | **Which lot an animal lives in — and a lot can hold other lots** | Slice 8b, and what replaces herds. **BOTH SIDES are `livestock_lots`**, because the thing contained is either a named animal (one head) or a smaller lot (fifty). Date-ranged, inclusive `ended_on`. **At most ONE OPEN membership per member**, partial unique index. **ONE LEVEL DEEP, enforced in `addLotToParent`** — a member may not hold things and a holder may not be a member; a CHECK cannot see other rows. `not_self` CHECK is the whole cycle guard, because one level makes longer loops unrepresentable. **No head event is ever written by a membership change** |
| `livestock_group_members` | Which lots are in a herd, **between which dates**. **BEING DROPPED — slice 8e.** Its SHAPE survived as `livestock_lot_members` rather than the table itself: retargeting in place would have broken herds immediately, and they must keep working until 8d ships the bulk move | Inclusive `ended_on`, matching `land_occupancy`. **At most ONE OPEN membership per lot across all herds**, partial unique index — a cow is not in two herds, and that is what makes moving between them one act. Composite FKs to both sides, CASCADE. **No head event is ever written by a membership change** |
