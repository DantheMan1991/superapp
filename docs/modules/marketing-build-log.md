# Marketing — build log archive

The older half of the Marketing build log, swept out of
[marketing.md](marketing.md) on 2026-09-11 when the dossier reached 2,804
lines and 78% of it was build log — the tax AGENTS.md warns about, since the
dossier is read at the start of every session that touches this module.

**Everything here is still the record.** `src/lib/build-docs.ts` walks the
whole tree, so this file renders at `/admin/docs` beside the dossier with no
code change. What stayed behind in the dossier is the current arc — the
several-websites work of 2026-09-10 and 2026-09-11 — plus the data model, the
seams, the decisions and the open items, which are never swept because they
describe the module as it is rather than how it got here.

Newest first, as the dossier's own log is: slice 18 back to slice 0.
