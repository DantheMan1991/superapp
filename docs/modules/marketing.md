...
0316 and 0317** (`social_posts` + its policies), applied to dev AND prod before
the merge; `verify-rls` reads 187 tables on both.

**Generated as `0309`/`0310` and renumbered FIVE TIMES** (→ 0311/0312 →
0313/0314 → 0315/0316 → 0316/0317) — Time slices 3, 4, 5 and 6 each took the next pair of slots
and merged first, on a day when four sessions were writing migrations against one
database. Each repair
...
- **THE DEV BRANCH HELD TWO TABLES PRODUCTION DID NOT** — resolved the same
  ...
  read 185 on dev and 183 on prod. Not this module's doing — `time_periods` and `time_sheets`
...
- **The crop is rendered, never stored, and the composite FK to the photo had
  ...
  **It came back within the day**: regenerating the migration after the
  renumber re-emitted the bare form, and that header comment is the only
  reason it was caught. Diff a regenerated migration against the one it
  replaces, always.
