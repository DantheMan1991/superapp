...
| Stripe webhook | Stripe signature | Only trusted source of billing state (S7). The PLATFORM charging the TENANT. Since back-office slice 5 (ADR 0043) it also posts each paid invoice and hour block into the OPERATOR tenant's books through Accounting's own verbs, as the operator's owner with no user — which workspace paid comes from OUR `subscriptions` row by customer id, never from the payload's metadata; idempotent on Stripe's id in `operator_postings` |
...
| `/health-check` | Public | Prospect funnel. The conversation is platform-level (`interview_sessions`, superadmin-only); **on promotion it is the fourth unauthenticated WRITE into a tenant, and the only one that does not name its tenant by a slug** — the OPERATOR tenant, named by `tenants.is_operator` (ADR 0041), which the visitor cannot choose. The session is claimed atomically first, then the write runs as `staff` with no user inside the operator's context through the shared party and Work doors and the leads slot (ADR 0042), so the member policies bound it. The operator is emailed at its own owners' addresses, never the visitor's. Caps unchanged. Treat all input as hostile |
| `/sites/[slug]`, `/hosted/[slug]` (tenant websites) | Public | **Unauthenticated by design** (ADR 0019). `slug → tenant` is one `withSystem` lookup returning identifiers; pages are then read in that tenant's context as `staff` through the member policies. Only the PUBLISHED snapshot is rendered, from typed sections, never markup. The one calendar a public page reads is the business's `Events` calendar (Marketing 9a), by id, through its workspace-wide share; no other calendar's rows reach a page. **A link an owner types is one of four shapes** — an in-site path (never `//host`), `http(s)://`, `mailto:`, `tel:` — refused by the schema on save and, at render, turned into plain words by `resolveHref` if a stored row holds anything else (`src/lib/sites/links.ts`; since Marketing 6c, before which a stored `javascript:` href was a stored XSS on this origin). `src/proxy.ts` rewrites `<slug>.<SITE_DOMAIN>` to `/hosted/…` |
...
| `GET /api/sites/slots` (a booking section's open times) | Public | **The third unauthenticated door into a tenant, a READ** (ADR 0025). `slug → tenant` through the trusted lookup, only a published site with Scheduling on, the section's rules from the PUBLISHED page, and the Bookings calendar's busy time read as `staff` with no user through that calendar's workspace-wide share. Answers instants and labels; everything else gets an empty list, so nothing here says which addresses exist. Capped per IP per hour in `public_access_attempts`, never cached |
| The booking form on any site route (`submitSiteBooking`) | Public | **An unauthenticated WRITE into a tenant, the enquiry's twin** (ADR 0025): honeypot, Zod, the same trusted lookup, refused unless the site is published and Scheduling is on. The chosen time is checked against the open times at the moment of writing under a per-calendar advisory lock, never trusted. Writes the party, the follow-up and the enquiry row through the same doors as the form, and the calendar item and attendee through the scheduling policies by way of the Bookings calendar's `write` share to everyone. Emails the business's own addresses only; nothing is sent to the address the visitor typed |
...
| `/sites/[slug]/map/[key]`, `/domain/[host]/map/[key]` (a site's map) | Public | **A PUBLISHED site's map only** (ADR 0026): a picture the platform draws from USGS Topo tiles around the geocoded pin, keyed by a hash of the pin, the zoom and the brand colour; a key the site does not hold is a 404. The SERVER fetches the tiles, and at save time the Census geocoder, with the business's own public address; a visitor's browser asks the platform for one image and nobody else for anything. The photo routes' public cache headers |
| `/api/marketing/sites/map/[key]` | Clerk session → `resolveTenantContext()` | The same picture for members: the draft preview, an unpublished site included |
| `/sites/[slug]/share/[key]` and the `/domain/[host]/` twin (a page's share image) | Public | **A PUBLISHED site only** (Marketing 11b): a 1200×630 picture the platform draws from the page's title as type paths, the brand colour and the logo's own pixels; the key is a hash of those, matched against the site's pages, and anything else is a 404. Nothing a tenant wrote reaches it as a file |
| An address a site used to have (`/sites/<old>/…`, `<old>.<SITE_DOMAIN>/…`) | Public | A permanent redirect to the same page at the current address, for as long as the old one is in `sites.previous_slugs` and nothing current is at it (Marketing 11b). The lookup is `withSystem`, identifiers only, like the slug lookup; an unpublished site is the same 404 as none |
...
| The page editor's assistant (`rewriteSectionAction`, `draftPageAction`, `describePhotoAction` in `src/modules/marketing/assistant-actions.ts`) | Clerk session, owner, Marketing enabled, `ANTHROPIC_API_KEY` set | **What leaves the platform** (Marketing 12, ADR 0027): the brief the public page already prints (name, tagline, kind, address, hours) plus the owner's own lines about the business (`settings.about`, written for the writer) and ONE thing: the words of one section by slot, one sentence about a page, or one photo's pixels. Never a row id, a visitor's message, a booking, a draft, a session or a file path. What comes back is words through one forced tool, cut to size and parsed through the content model, handed to the editor's UNSAVED state: these actions write nothing, the owner saves. A valve of sixty asks per tenant per hour (`site_assistant` in `public_access_attempts`) |
| A pack's block on a public page (`block` sections; Marketing 9b, ADR 0028) | Public | The site reads a pack's rows THROUGH THE PACK'S OWN VERBS (`src/lib/site-blocks/resolve.ts` → the provider in `registry.ts`), inside the page's anonymous tenant transaction, only for a block a page on show carries and only while the pack is switched on for that tenant (`tenant_modules`, read in the same transaction). Retail's price list touches `retail_channels`, `retail_prices`, `inventory_items` and `inventory_movements` under their tenant-only member policies; what reaches the page is names, prices and "sold out", never a cost, a quantity or a lot. A provider answers rows and the site draws them: no pack puts markup or script on a public origin |
| `/api/marketing/sites/live` (POST) | Clerk session → `resolveTenantContext()` | The editor's live preview (Marketing 13, ADR 0029): the views of the pack blocks and the events an UNSAVED page's sections would show, read through the same two functions the draft route uses (`loadLiveData`), inside the member's tenant transaction. The sections come from the client and are parsed through the content model before anything reads them; nothing is written; `no-store`. The draft frame itself (`/sites/[slug]/draft/…?live=1`) believes a `yosher:site-draft` message only from its parent window on this origin, checks its shape, and draws it with the same renderer as the saved draft |
| `/dashboard/m/marketing/website/photos` (the shot list) | Clerk session → `requireTenant()`, Marketing enabled | Where a photo belongs on the tenant's own draft pages (Marketing 18, ADR 0031), read inside the member's tenant transaction on every load and stored nowhere; staff read, owners place. The notes beside each place are the industry template's data, never a tenant's |
