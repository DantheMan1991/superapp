-- Maintenance: schedules, meter readings and the services actually performed.
--
-- HAND-REORDERED FROM THE GENERATED OUTPUT, for the second time in this pack
-- and for a reason worth stating generally:
--
--   drizzle-kit emits EVERY foreign key before EVERY index. A composite FK
--   whose target unique index is created in the SAME migration therefore fails
--   with "there is no unique constraint matching given keys".
--
-- 0125 hit this self-referentially (assets pointing at itself). This one hits
-- it across tables: `ame_schedule_fk` references
-- asset_maintenance_schedules(tenant_id, id), whose unique index the generator
-- placed eight statements later. Cross-table FKs to a PRE-EXISTING table are
-- fine, which is why this only bites when the target is new too.
--
-- Order here: tables, then unique indexes, then foreign keys, then the rest.
