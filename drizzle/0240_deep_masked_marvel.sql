...
--      and generated_through <> (next_run_date - interval '1 month')::date;
-- A row listed was either forward-edited since (expected, safe — the audit
-- row ledger.recurring_updated says so) or generated in the window (unsafe:
-- set generated_through to next_run_date - 1 month by hand).
