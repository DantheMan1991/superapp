...
/**
 * THE MODULE CATALOGUE. Extracted from `scripts/seed.ts` on 2026-09-14 so that
 * something other than the seed can read it.
 *
 * `packs-and-profiles.md` had recorded the need since Layer 2 shipped: *"Nothing
 * asserts a pack in code has a seed row in `scripts/seed.ts`. That file calls
 * `main()` at module load, so a test cannot import its catalogue without opening
 * a database connection."* Two things now do — `tests/module-catalogue.test.ts`,
 * which fails if a registered pack has no row here, and
 * `scripts/verify-modules.ts`, which fails if a row here is missing from a
 * database.
 *
 * The second one is why this happened when it did: the `jobs` pack shipped to
 * production with every table migrated and RLS verified, and no row in
 * `modules` — so nobody could switch it on and nobody could see it.
 *
 * **A TYPE-ONLY IMPORT of the schema**, deliberately: this file must stay
 * importable by a test without dragging a database driver in behind it, which is
 * the whole reason it is separate from the seed.
 */
 ...
 */
export const MODULES: (typeof schema.modules.$inferInsert)[] = [
  {
    id: "hello",
    name: "Hello Module",
    description:
