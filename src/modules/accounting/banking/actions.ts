...
import { quickAddTransaction } from "./quick-add";
import {
...
export async function quickAddTransactionAction(
  ...
  try {
    /*
     * THE RULE MOVED DOWN, and this is the thin thing that was left.
     *
     * Slice C1 needs the same posting from a SENTENCE, and a tell source
     * records through the module's own verb inside the transaction the platform
     * opened for it — which a server action cannot be, because it opens its own
     * and reads its own session. So `quick-add.ts` owns the lines, the statuses
     * and the refusals, and both doors call it.
     */
    const done = await inTenant(ctx, async (tx) => {
      const result = await quickAddTransaction(tx, ctx, p);
      await logAuditInTx(tx, {
        action: result.status === "posted" ? "ledger.entry_posted" : "ledger.entry_drafted",
        tenantId: ctx.tenantId,
        actorClerkUserId: ctx.userId,
        ...
        meta: {
          ...
          ...(result.otherRegisterId ? { toBankAccountId: result.otherRegisterId } : {}),
        },
      });
      return result;
    });
    revalidateBanking(p.bankAccountId);
    if (done.otherRegisterId) revalidateBanking(done.otherRegisterId);
    return { ok: true };
  } catch (err) {
    return fail(err);
  }
