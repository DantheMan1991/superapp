...
// ------------------------------------------------------------------ herds ---
//
// **GONE.** Slice 8e converted every herd into a LOT holding what it held, and
// nothing here reads `livestock_groups` any more. The tables still exist until
// the DROP ships in its own migration — `main` auto-deploys and nothing applies
// migrations for it, so removing the code and the tables in one release would
// take the running app down between the two.
//
// What a herd did is now in "animals live in a lot" below, and the bulk move
// that was its only unique power is `moveLotsToZone` under occupancy.
