// Test files that talk to the database.
//
// These run SEQUENTIALLY (see vitest.config.ts): they share one Neon branch,
// create and delete tenants, and several assert what is NOT visible across
// tenants — an assertion another file writing at the same moment can break.
// Everything else is pure and runs in parallel.
//
// THIS LIST IS CHECKED, NOT TRUSTED. tests/db-backed-files.test.ts recomputes
// it from file contents and fails if it has drifted, so adding a database test
// to a file that was pure cannot silently start racing.
export const DB_BACKED_TESTS = [
  "tests/attention-sources.test.ts",
  "tests/time-attention.test.ts",
  "tests/time-labor-posting.test.ts",
  "tests/time-pin-ops.test.ts",
  "tests/time-tell-source.test.ts",
  "tests/banking-personal-db.test.ts",
  "tests/banking-payees-db.test.ts",
  "tests/banking-rules-db.test.ts",
  "tests/books-start-db.test.ts",
  "tests/banking.test.ts",
  "tests/cash-basis-db.test.ts",
  "tests/deposits.test.ts",
  "tests/credit-memos.test.ts",
  "tests/close.test.ts",
  "tests/contact.test.ts",
  "tests/operator-relationship.test.ts",
  "tests/operator-provision.test.ts",
  "tests/support-sessions.test.ts",
  "tests/platform-revenue.test.ts",
  "tests/health-signals.test.ts",
  "tests/profile-seed.test.ts",
  "tests/jobs-allowance.test.ts",
  "tests/jobs-bim-schedule-ops.test.ts",
  "tests/jobs-bim-takeoff-ops.test.ts",
  "tests/jobs-outline-ops.test.ts",
  "tests/jobs-walk-ops.test.ts",
  "tests/engagements-ops.test.ts",
  "tests/engagement-onboarding.test.ts",
  "tests/discovery-ops.test.ts",
  "tests/crm-automation-ops.test.ts",
  "tests/crm-merge-ops.test.ts",
  "tests/crm-reports-ops.test.ts",
  "tests/crm-views-ops.test.ts",
  "tests/documents-dms/accounting-overlap.test.ts",
  "tests/documents-dms/folder-ops.test.ts",
  "tests/documents-dms/inbound.test.ts",
  "tests/documents-dms/search.test.ts",
  "tests/documents-dms/shares.test.ts",
  "tests/documents-dms/tags-and-views.test.ts",
  "tests/documents-dms/versions.test.ts",
  "tests/documents-templates.test.ts",
  "tests/documents.test.ts",
  "tests/identity-account-deletion.test.ts",
  "tests/email.test.ts",
  "tests/entity-scope-policies.test.ts",
  "tests/entities-db.test.ts",
  "tests/export.test.ts",
  "tests/interview.test.ts",
  "tests/leads-db.test.ts",
  "tests/invoice-reminders-db.test.ts",
  "tests/invoicing.test.ts",
  "tests/asset-opening-db.test.ts",
  "tests/assets-depreciation-posting.test.ts",
  "tests/assets-maintenance-work.test.ts",
  "tests/assets-ops.test.ts",
  "tests/land-ops.test.ts",
  "tests/enterprises-ops.test.ts",
  "tests/enterprise-posting.test.ts",
  "tests/inventory-ops.test.ts",
  "tests/inventory-posting.test.ts",
  "tests/inventory-attention-db.test.ts",
  "tests/documents-attachments.test.ts",
  "tests/feedback-db.test.ts",
  "tests/livestock-attention.test.ts",
  "tests/livestock-ops.test.ts",
  "tests/production-ops.test.ts",
  "tests/retail-ops.test.ts",
  "tests/isolation/access-levels.test.ts",
  "tests/isolation/accounting.test.ts",
  "tests/isolation/assets.test.ts",
  "tests/isolation/banking.test.ts",
  "tests/isolation/deposits.test.ts",
  "tests/isolation/credit-memos.test.ts",
  "tests/isolation/brand.test.ts",
  "tests/isolation/close.test.ts",
  "tests/isolation/constraints.test.ts",
  "tests/isolation/core.test.ts",
  "tests/isolation/crm-rule-health.test.ts",
  "tests/isolation/crm-settings.test.ts",
  "tests/isolation/crm.test.ts",
  "tests/isolation/documents-dms.test.ts",
  "tests/isolation/device-grants.test.ts",
  "tests/isolation/documents.test.ts",
  "tests/isolation/feedback.test.ts",
  "tests/isolation/interview.test.ts",
  "tests/isolation/entity-scope.test.ts",
  "tests/isolation/enterprises.test.ts",
  "tests/isolation/inventory.test.ts",
  "tests/isolation/land.test.ts",
  "tests/isolation/livestock.test.ts",
  "tests/isolation/mail.test.ts",
  "tests/isolation/membership.test.ts",
  "tests/isolation/notifications.test.ts",
  "tests/isolation/parties.test.ts",
  "tests/isolation/payments.test.ts",
  "tests/isolation/production.test.ts",
  "tests/isolation/professional-services.test.ts",
  "tests/jobs-ops.test.ts",
  "tests/isolation/jobs.test.ts",
  "tests/isolation/push.test.ts",
  "tests/isolation/retail.test.ts",
  "tests/isolation/operator.test.ts",
  "tests/isolation/payables.test.ts",
  "tests/isolation/retainer.test.ts",
  "tests/isolation/scheduling.test.ts",
  "tests/isolation/sites.test.ts",
  "tests/isolation/social.test.ts",
  "tests/isolation/support.test.ts",
  "tests/isolation/platform-revenue.test.ts",
  "tests/isolation/shares.test.ts",
  "tests/isolation/work.test.ts",
  "tests/isolation/time.test.ts",
  "tests/ledger.test.ts",
  "tests/mail-inbox.test.ts",
  "tests/mailbox.test.ts",
  "tests/nesting-parent-fk.test.ts",
  "tests/opening-position.test.ts",
  "tests/payables.test.ts",
  "tests/paste-targets-db.test.ts",
  "tests/reports.test.ts",
  "tests/retainer-billing.test.ts",
  "tests/sales-tax-db.test.ts",
  "tests/scheduling-attention.test.ts",
  "tests/scheduling-availability-db.test.ts",
  "tests/scheduling-calendar-ops.test.ts",
  "tests/scheduling-recurring-range.test.ts",
  "tests/scheduling-item-ops.test.ts",
  "tests/setup-interview-db.test.ts",
  "tests/setup-sources-db.test.ts",
  "tests/device-grants-redeem.test.ts",
  "tests/inventory-tell-source.test.ts",
  "tests/tell-catalogue-db.test.ts",
  "tests/tell-sources-db.test.ts",
  "tests/work-tell-source.test.ts",
  "tests/jobs-tell-source.test.ts",
  "tests/work-attention.test.ts",
  "tests/work-ops.test.ts",
  "tests/work.test.ts",
];
