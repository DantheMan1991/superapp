...
d("livestock tables (RLS)", () => {
  ...
  // ---- herds: DROPPED in slice 8g ----------------------------------------
  //
  // `livestock_groups` and `livestock_group_members` no longer exist (`0232`).
  // Every herd became a LOT holding what it held in 8e, one release earlier, and
  // the certification for that shape is "animals live in a lot" below.
  //
  // These tests stayed alive through 8e ON PURPOSE — the tables were still there
  // and a live table with no certification is exactly what this suite is for.
  // They go now, with the tables.
