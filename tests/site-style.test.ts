...
describe("a section's style", () => {
  ...
  it("resolves the owner's choices over the kind's defaults, and the renderer's adjustment over both", () => {
    ...
    expect(Object.keys(SECTION_DEFAULTS).sort()).toEqual(["about", "block", "booking", "columns", "contact", "cta", "events", "form", "gallery", "hero", "hours", "image", "map", "offer", "slideshow", "text"]);
  });
