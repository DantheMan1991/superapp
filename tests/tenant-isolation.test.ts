...
/**
 * memberships.role is the one column in the schema that a background job reads
 * to decide WHOSE BEHALF it may act on. That makes "owner" a capability, and
 * this block is the certification that tenant context cannot mint one.
 *
 * Before drizzle/0085, the member UPDATE policy from 0018 constrained only the
 * tenant, not the value — any withTenant transaction could write role='owner'
 * or rewrite an existing owner row, with requireTenantOwner in app code as the
 * only guard. Harmless while nothing read the column for authorization; an
 * escalation the moment the notifications digest does.
 */
...
/**
 * The two notification tables (0089/0090).
 *
 * `notification_preferences` is the first table in the product whose write rule
 * is "your own row and nobody else's", enforced by policy rather than by a
 * server action remembering to check. That claim is only worth making if a
 * colleague genuinely cannot write your row, which is what this block asserts.
 *
 * `notification_digest_log` is superadmin-only on purpose: the rows say how
 * much outstanding work each person carried on each day, which is a
 * performance record rather than business data.
 */
...
/**
 * Rule health (0091/0092).
 *
 * The asymmetry is the whole design: every member may READ it, because a rule
 * that has been failing since Tuesday is part of the answer to "what changed my
 * record?" — but NOBODY writes it from tenant context, so a member cannot forge
 * a failure against a rule they would rather nobody used. The engine writes it
 * under withSystem.
 */
...
/**
 * crm_settings (0093/0094) — the AI cooldown row.
 *
 * Member-all, which is a wider grant than the notification tables got. It is
 * justified only if the tenant scoping actually holds, because the whole point
 * of the grant is that the extractor can CLAIM the cooldown inside the caller's
 * own transaction.
 */
d("crm settings (RLS)", () => {
